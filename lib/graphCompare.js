...
handler.prototype.build = function(row, rows) {
    ...
    for (var i = 0, l = fields.length; i < l; i++) {
        field = fields[i];
        field.pct = require('./util').percent(field.value, total);
        field.offset = offset;
        ...
        field.value = require('./util').ac(field.value);

        if (!colorCache[field.field]) {
            // Generate "unique" value from md5 hash.
            if (this.settings.colorsUnique) {
                hash = require('crypto').createHash('md5').update(field.field).digest('hex').substr(0, 6);
    ...
    if (this.settings.scale && row.value && row.value.type) {
        display.pct = require('./util').percent(total, this.getMax(rows));
    }
    else {
        display.pct = 100;
    }
