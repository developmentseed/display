 ...
 *   undefined in the data will be rendered as 0%.
 */

function handler (schema, identifier, config) {
    require('./base').call(this, schema, identifier, config);
}
...
handler.prototype.build = function(row, rows) {
    ...
    for (var i = 0; i < fields.length; i++) {
        ...
        else if (this.settings.includeEmpty) {
            display.values.push(util.percent(0, denominator) + '%');
        }
    }
